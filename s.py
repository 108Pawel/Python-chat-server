...
def parseMessage(command):
    ...
    #removing the word message: (8 haracters)
    keyValuePair = command[8:len(command)] 
    # finding hash which is just behind - sign
    dashPosition = keyValuePair.index('-')
    asterixPosition = keyValuePair.index('*')

    hash = keyValuePair[dashPosition+1:len(keyValuePair)]
    message = keyValuePair[asterixPosition+1:dashPosition]
    ...
    print " the messege is.... " + message 

    if hash in newHash:
        print "all ok"


def parseUsers(command):
    print "Printing logged in person"
    #removing the word message: (8 haracters)
    keyValuePair = command[8:len(command)] 
    # finding hash which is just behind - sign
    dashPosition = keyValuePair.index('-')
